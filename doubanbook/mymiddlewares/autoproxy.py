...
class AutoProxyMiddleware(object):
    ...
    def process_response(self, request, response, spider):
        ...
        if self.ban_re:
            ...
            if match:
                self.invaild_proxy(request.meta['proxy'])
                logger.debug(
                    "Proxy[%s] ban because pattern match:[%s]. ", request.meta['proxy'], str(match))
                new_request = request.copy()
                new_request.dont_filter = True
                return new_request
    ...
    def change_proxy(self):
        ...
        while True:
            ...
            if self.len_valid_proxy() == 0:
                logger.info(
                    'Available proxys is none.Waiting for fetch new proxy.')
                break
        logger.info('Change proxy to %s', self.proxy[self.proxy_index])
        logger.info('Available proxys[%s]: %s',
                    self.len_valid_proxy(), self.valid_proxyes())
    ...
    def check_proxy(self, proxy):
        ...
        try:
            for url, code in self.autoproxy.test_urls:
                resbody = opener.open(
                    url, timeout=self.autoproxy.test_proxy_timeout).read()
    ...
    def fetch_proxy_from_ip3336(self):
        ...
        try:
            for i in range(1, 6):
                soup = self.get_soup(url + str(i))
                trs = soup.find(
                    "div", attrs={"id": "list"}).table.find_all("tr")
                for i, tr in enumerate(trs):
                    if 0 == i:
                        continue
                    tds = tr.find_all("td")
    ...
    def fetch_proxy_from_kxdaili(self):
        ...
        try:
            for i in range(1, 11):
                ...
                trs = soup.find(
                    "table", attrs={"class": "ui table segment"}).find_all("tr")
                for i, tr in enumerate(trs):
                    if 0 == i:
                        continue
                    tds = tr.find_all("td")
            ...
            logger.error(
                'Failed to fetch_proxy_from_kxdaili. Exception[%s]', e)
